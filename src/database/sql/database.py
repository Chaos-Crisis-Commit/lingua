# pylint: disable = no-name-in-module,no-member,logging-fstring-interpolation,broad-exception-raised,raise-missing-from
"""
...
class Database():
    ...
    def __init__(self, db_name: str) -> None:
        file_path = os.path.join(os.getcwd(), 'data', f'{db_name}.db')
        self.connector = sqlite3.connect(file_path)
    ...
    def select_data(self, query_for: str, query_table: str, where: str=""):
        ...
        """
        query_string = f"SELECT {query_for} FROM {query_table} WHERE {where};"
        if where:
            try:
